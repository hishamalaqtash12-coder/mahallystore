      ...
      <div className="p-4 border-t border-zinc-100">
        {/* <div className="flex items-center gap-3 px-2 mb-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[12px] font-bold text-zinc-900 truncate leading-none mb-1">{customerName || user?.displayName || (isAr ? "تاجر" : "Merchant")}</p>
            <div className="flex items-center gap-1.5">
              <p className="text-[11px] text-zinc-500 truncate leading-none">{isAr ? "مركز التجار" : "Seller Central"}</p>
        ...
        {isVendor && !isApprovedVendor && (
          <div className="px-2 py-2 mb-2 bg-amber-50 border border-amber-100 rounded-md">
            <p className="text-[10px] text-amber-700 leading-tight">
              <strong>{isAr ? "المتجر غير متصل:" : "Store Offline:"}</strong> {isAr ? "طلب تسجيلك قيد المراجعة حالياً. سيتم إعلامك فور الموافقة عليه." : "Your registration is being reviewed. You will be notified once approved."}
            </p>
          </div>
        )}
        <button
