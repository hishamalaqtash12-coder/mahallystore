...
export default function ReviewSubmissionModal({ isOpen, onClose, order, user, userId, reviewedProducts = [], onReviewSubmitted }) {
    ...
    try {
      const res = await fetch("/api/reviews", {
        ...
        body: JSON.stringify({
          ...
          rating: rating,
          review: reviewText,
          reviewer: customerName || user?.displayName || user?.phoneNumber || t("anonymousCustomer"),
  ...
  const getRatingLabel = (rating) => {
    const labels = {
      1: t("ratingLabels.hateIt"),
      2: t("ratingLabels.dontLikeIt"),
      3: t("ratingLabels.itsOkay"),
      4: t("ratingLabels.likeIt"),
      5: t("ratingLabels.loveIt")
    };
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-zinc-200 flex items-center justify-between bg-zinc-50">
          <h2 className="text-[17px] font-bold text-zinc-900">
            {success ? t("submittedTitle") : step === 1 ? t("selectProductTitle") : `${t("reviewing")}: ${selectedProduct.name}`}
          </h2>
        ...
        <div className="flex-1 overflow-y-auto p-8">
          {success ? (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <h3 className="text-[24px] font-bold text-zinc-900 mb-2">{t("thankYou")}</h3>
              <p className="text-[15px] text-zinc-500 max-w-md">{t("feedbackMessage")}</p>
              <button
                onClick={onClose}
            ...
            <div className="space-y-6">
              <p className="text-[15px] text-zinc-600">
                {t("selectProductPrompt", { orderId: order.id })}
              </p>
              {reviewableItems.length === 0 ? (
                <div className="text-center py-10 text-zinc-400">
                  <CheckCircle2 size={40} className="mx-auto mb-3 text-emerald-400" />
                  <p className="font-bold text-zinc-700">{t("allReviewed")}</p>
                  <p className="text-[13px] mt-1">{t("allReviewedDesc")}</p>
                </div>
              ) : (
                    ...
                    >
                      ...
                      <div className="flex-1 text-start">
                        ...
                        <p className="text-[12px] text-zinc-400">
                          {t("soldBy")} {item.meta_data?.find(m => m.key === "merchant_name")?.value || t("mahallyPartner")}
                        </p>
                      </div>
                      <ChevronRight size={18} className={`text-zinc-300 group-hover:text-brand ${isAr ? 'rotate-180' : ''}`} />
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-8">
              {/* Overall Rating */}
              <div>
                <h3 className="text-[18px] font-bold text-zinc-900 mb-2">{t("overallRating")}</h3>
              ...
              <div className="space-y-4">
                <h3 className="text-[18px] font-bold text-zinc-900">{t("writtenReview")}</h3>
                <textarea
                  value={reviewText}
