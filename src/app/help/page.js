...
// --- Design Tokens ---
const PRIMARY_COLOR = "#007185";
const ACCENT_COLOR = "#FF9900";
    ...
    <div className="min-h-screen bg-white font-sans text-zinc-800 text-xs">
          ...
          <div className="text-center">
            ...
            <div className="max-w-[500px] mx-auto relative">
              <form
                ...
                className={`flex items-center bg-white border rounded-md p-0.5 transition-all ${isSearchFocused ? 'border-[#007185] ring-2 ring-[#007185]/10' : 'border-zinc-300'}`}
              >
                ...
                <button
                  type="submit"
                  className="bg-[#007185] text-white px-5 py-1.5 rounded-sm font-semibold hover:bg-[#005a6a] transition-all text-xs"
                >
                  بحث
                </button>
              </form>
      ...
      <div className="max-w-[1000px] mx-auto px-4 py-8 flex flex-col md:flex-row gap-8">
        ...
        <aside className="w-full md:w-[240px] shrink-0 space-y-6">
          ...
          >
            <div>
              <div className="text-[10px] font-bold text-[#007185] mb-0.5">سجل الدعم</div>
            ...
            <div className="w-8 h-8 bg-[#007185] rounded-md flex items-center justify-center text-white">
          ...
          <div>
            <h2 className="text-sm font-bold text-zinc-950 mb-3 px-1 flex items-center gap-2">
              <span className="w-1 h-4 bg-[#007185] rounded-sm inline-block" />
              ...
              {HELP_TOPICS.map((topic) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-right flex items-center justify-between px-3 py-2 rounded-md transition-all ${isActive ? 'bg-[#007185]/10 text-[#007185] font-bold shadow-sm' : 'hover:bg-zinc-50 text-zinc-600'}`}
                  >
                    <div className="flex items-center gap-3">
                      <topic.icon size={14} className={isActive ? 'text-[#007185]' : 'text-zinc-400'} />
              ...
              <div className="space-y-3">
                {filteredFaqs.map((faq) => (
                  <div key={faq.id} className="bg-white rounded-lg border border-zinc-200 overflow-hidden">
                    ...
                    >
                      <h3 className="text-xs font-bold text-zinc-900 group-hover:text-[#007185] transition-colors pl-8 leading-snug">
                        {faq.q}
                      </h3>
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${expandedItems[faq.id] ? 'bg-[#007185] text-white rotate-180' : 'bg-zinc-100 text-zinc-400'}`}>
                        <ChevronDown size={14} />
                      </div>
                    </button>
                ...
                {filteredFaqs.length === 0 && (
                  <div className="text-center py-16 bg-zinc-50 rounded-lg border border-dashed border-zinc-200">
                    ...
                    <button
                      onClick={() => { setSearchQuery(""); router.push("/help"); }}
                      className="text-[#007185] font-bold hover:underline"
                    >
            ...
            <div>
              ...
              <div className="mt-12 p-6 rounded-xl bg-zinc-50 border border-zinc-200 text-center">
                ...
                <div className="flex justify-center">
                  <Link
                    href="/messages?to=admin"
                    className="bg-[#007185] text-white px-6 py-2 rounded-sm font-bold hover:bg-[#005a6a] transition-all flex items-center gap-2 text-xs"
                  >
...
export default function HelpPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex justify-center items-center"><div className="w-6 h-6 border-2 border-[#007185] border-t-transparent rounded-full animate-spin"></div></div>}>
      <HelpContent />
    </Suspense>
  );
}
