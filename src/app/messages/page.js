      ...
      if (part.match(urlRegex)) {
        return (
          <a key={i} href={part} target="_blank" rel="noopener noreferrer" 
             className={`underline transition-colors hover:opacity-80 ${isMe ? 'text-blue-100 font-medium' : 'text-[#007185] font-medium'}`} 
        ...
        <div className="px-5 py-4 border-b border-zinc-200">
          <div className="flex items-center justify-between mb-3">
            <h1 className="text-[18px] font-semibold text-zinc-900 flex items-center gap-2">
              <MessageCircle size={18} className="text-[#007185]" />
              الرسائل
            </h1>
            <div className="flex gap-1.5">
              <button
                onClick={async () => { setRefreshingConvs(true); await fetchData(); setRefreshingConvs(false); }}
                className={`w-8 h-8 border border-zinc-200 rounded-md flex items-center justify-center text-zinc-500 hover:border-[#007185] hover:text-[#007185] transition-all ${refreshingConvs ? "animate-spin text-[#007185]" : ""}`}
              ...
              <button
                ...
                className="w-8 h-8 border border-zinc-300 rounded-md flex items-center justify-center text-zinc-600 hover:bg-[#FFD814] hover:border-[#FCD200] transition-all shadow-sm"
              >
                <Plus size={16} />
              </button>
            </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full h-[31px] pl-9 pr-3 bg-zinc-50 border border-zinc-300 rounded-md text-[13px] outline-none focus:border-[#e77600] transition-all"
            />
          </div>
          <div className="flex items-center mt-3 px-0.5 select-none">
            ...
            >
              <span className={`w-1.5 h-1.5 rounded-full ${showUnreadOnly ? "bg-[#e77600] animate-pulse" : "bg-zinc-300"}`} />
              غير المقروءة فقط
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-3 px-3 space-y-0.5">
          {error && (
            <div className="mb-3 p-2.5 bg-zinc-50 border border-zinc-200 rounded-md flex items-start gap-2">
              <AlertCircle size={14} className="text-[#007185] shrink-0 mt-0.5" />
          ...
          {!isAdmin && String(wooId) !== "1" && (!showUnreadOnly || adminUnreadCount > 0) && (
            <div
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-md cursor-pointer transition-all ${(vendorId === "admin" || String(vendorId) === "1") ? "bg-[#e8f4f5] border border-[#b2d8dc]" : "hover:bg-zinc-50 border border-transparent"}`}
            >
              ...
              <div className="flex-1 min-w-0 flex items-center justify-between">
                <div>
                  <p className={`text-[13px] font-medium ${(vendorId === "admin" || String(vendorId) === "1") ? "text-[#007185]" : "text-zinc-900"}`}>دعم Mahally</p>
                  <p className="text-[12px] text-zinc-500 truncate mt-0.5">مدير النظام</p>
                </div>
                {adminUnreadCount > 0 && (
                  <span className="flex items-center justify-center min-w-[20px] h-5 px-1.5 bg-[#e77600] text-white text-[10px] font-bold rounded-full shadow-sm animate-pulse shrink-0 ml-2">
                    {adminUnreadCount}
                  </span>
                )}
              </div>
              ...
              .map((conv, index) => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-md cursor-pointer transition-all border ${isActive ? "bg-[#e8f4f5] border-[#b2d8dc]" : "border-transparent hover:bg-zinc-50 hover:border-zinc-200"}`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-baseline">
                        <p className={`text-[13px] font-medium truncate ${isActive ? "text-[#007185]" : "text-zinc-900"}`}>{conv.name}</p>
      ...
      <div className="flex-1 flex min-w-0 relative">
        ...
        {isChatLoading && vendorId && (
          <div className="absolute inset-0 bg-white/60 backdrop-blur-sm z-[60] flex flex-col items-center justify-center transition-all duration-300">
            <div className="w-8 h-8 border-4 border-zinc-200 border-t-[#007185] rounded-full animate-spin mb-3"></div>
        ...
        {vendorId ? (
          <>
            ...
            <header className="h-[56px] flex items-center justify-between px-5 border-b border-zinc-200 bg-white shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-[15px] font-semibold text-zinc-900 flex items-center gap-1.5">
                    ...
                    {vendor?.isVerified && <BadgeCheck size={14} className="text-[#007185]" />}
                  </h2>
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className={`w-8 h-8 border border-zinc-200 rounded-md flex items-center justify-center text-zinc-500 hover:text-[#007185] transition-all ${refreshingMsgs ? "animate-spin" : ""}`}
                  title="تحديث الرسائل"
                >
                  <RefreshCw size={15} />
                </button>
                <button
                  ...
                  className={`w-8 h-8 border rounded-md flex items-center justify-center transition-all ${showInfo ? "bg-[#e8f4f5] border-[#b2d8dc] text-[#007185]" : "border-zinc-200 text-zinc-500 hover:text-[#007185]"}`}
                >
                  <Info size={15} />
                </button>
              </div>
                    ...
                    return (
                      <div key={msg.id || `msg-${index}`} className={`flex flex-col ${isMe ? "items-end" : "items-start"} group relative`}>
                        <div
                          ...
                          className={`message-bubble-wrapper max-w-[75%] lg:max-w-[65%] px-4 py-2.5 rounded-lg text-[13px] leading-relaxed border relative transition-all ${isSelected ? "ring-2 ring-[#007185] ring-offset-1" : ""
                            } ${isMe ? "bg-[#007185] text-white border-[#007185]" : "bg-white text-zinc-800 border-zinc-200 shadow-sm"}`}
                        >
                          {msg.replyTo && (
                            <div className={`mb-2 p-2 border-l-2 text-[11px] rounded-sm ${isMe ? "border-white/40 bg-white/10 text-white/60" : "border-[#007185] bg-zinc-50 text-zinc-500"}`}>
                              ↩ {msg.replyTo.text}
                            </div>
                          )}
                          ...
                          {msg.customMeta?.type === "product" && !msg.isDeleted && (
                            <div className={`mt-2 p-2.5 rounded-md border flex gap-2.5 ${isMe ? "bg-white/10 border-white/20" : "bg-zinc-50 border-zinc-200"}`}>
                              ...
                              <div>
                                ...
                                <p className={`text-[12px] font-semibold mt-0.5 ${isMe ? "text-white" : "text-[#007185]"}`}>د.أ {msg.customMeta.price}</p>
                              </div>
                            </div>
                          )}
                          ...
                          {!msg.isDeleted && (
                            <div className={`absolute -top-9 ${isMe ? "right-0" : "left-0"} flex gap-1 bg-white border border-zinc-200 p-1 shadow-md rounded-md z-50 transition-all ${isSelected ? "opacity-100 visible" : "opacity-0 invisible group-hover:opacity-100 group-hover:visible"}`}>
                              <button onClick={(e) => { e.stopPropagation(); setReplyTo(msg); setSelectedMessageId(null); }} className="w-7 h-7 hover:bg-zinc-50 flex items-center justify-center text-zinc-400 hover:text-[#007185] rounded transition-all" title="الرد">
                                <Reply size={13} />
                              </button>
                              <button onClick={(e) => { e.stopPropagation(); setShowReactionPicker(showReactionPicker === msg.id ? null : msg.id); }} className="w-7 h-7 hover:bg-zinc-50 flex items-center justify-center text-zinc-400 hover:text-[#007185] rounded transition-all" title="تفاعل">
                                <Smile size={13} />
                              </button>
                              {isMe && (
            ...
            <div className="px-5 py-3 border-t border-zinc-200 bg-white shrink-0">
              ...
              {selectedFile && (
                <div className="flex items-center justify-between px-3 py-2 bg-[#e8f4f5] border border-b-0 border-[#b2d8dc] rounded-t-md">
                  <div className="flex items-center gap-2">
                    {previewUrl
                      ? <img src={previewUrl} className="w-7 h-7 object-cover rounded border border-white" alt="preview" />
                      : <File size={14} className="text-[#007185]" />
                    }
                    <span className="text-[12px] text-[#007185] truncate max-w-[200px]">{selectedFile.name}</span>
                  </div>
              ...
              <div className={`flex items-end gap-2 bg-zinc-50 border border-zinc-300 px-2 py-1.5 ${replyTo || selectedFile ? "rounded-b-md" : "rounded-md"}`}>
                <button onClick={() => fileInputRef.current?.click()} className="w-8 h-8 flex items-center justify-center text-zinc-400 hover:text-[#007185] transition-colors">
                  <Paperclip size={16} />
                </button>
                <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileChange} />

                <div className="relative shrink-0" ref={emojiRef}>
                  <button onClick={() => setShowEmoji(!showEmoji)} className="w-8 h-8 flex items-center justify-center text-zinc-400 hover:text-[#007185] transition-colors">
                    <Smile size={16} />
                  </button>
                  {showEmoji && (
                ...
                <button
                  onClick={() => handleSend()}
                  disabled={sending}
                  className="h-[34px] px-4 bg-[#FFD814] hover:bg-[#F7CA00] border border-[#FCD200] rounded-md text-[13px] font-medium shadow-sm transition-all disabled:opacity-50 flex items-center gap-1.5 shrink-0"
                >
              ...
              <div className="flex justify-end mt-1.5 px-1">
                <label className="flex items-center gap-1.5 text-[11px] text-zinc-500 cursor-pointer hover:text-zinc-700 transition-colors select-none">
                  <input 
                    ...
                    className="rounded border-zinc-300 w-3 h-3 text-[#007185] focus:ring-[#007185]" 
                  />
                  اضغط Enter للإرسال
                </label>
              </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            ...
            <button
              onClick={() => setShowNewChat(true)}
              className="h-[34px] px-6 bg-[#FFD814] hover:bg-[#F7CA00] border border-[#FCD200] rounded-md text-[13px] font-medium shadow-sm transition-all"
            >
              ابحث عن جهة اتصال
            </button>
          </div>
            ...
            {isAdminAccount ? (
              ...
              ["info", "products", "orders"].map(tab => (
                <button
                  ...
                  className={`px-3 py-2.5 text-[13px] font-medium capitalize border-b-2 transition-colors ${activeTab === tab ? "border-[#e77600] text-zinc-900" : "border-transparent text-zinc-500 hover:text-zinc-800"} ${tab === "products" && (vendor?.role === "customer" || vendorProducts.length === 0) ? "hidden" : ""}`}
                >
              ...
              <div className="space-y-4">
                <div className="text-center">
                  <div className="w-16 h-16 bg-zinc-900 rounded-lg flex items-center justify-center mx-auto mb-3 border border-zinc-800">
                    <ShieldAlert size={28} className="text-[#007185]" />
                  </div>
                  <h3 className="text-[15px] font-semibold text-zinc-900">دعم Mahally</h3>
                  <span className="inline-flex items-center gap-1 mt-1.5 bg-[#e8f4f5] text-[#007185] text-[11px] font-medium px-2.5 py-0.5 rounded-full border border-[#b2d8dc]">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-4">
                      <div className="text-center">
                        ...
                        <span className={`inline-flex items-center gap-1 mt-1.5 text-[11px] font-medium px-2.5 py-0.5 rounded-full border ${vendor?.role === "customer" ? "bg-zinc-100 text-zinc-600 border-zinc-200" : "bg-[#e8f4f5] text-[#007185] border-[#b2d8dc]"}`}>
                      ...
                      <div className="p-3 bg-zinc-50 border border-zinc-200 rounded-md space-y-2">
                        ...
                        {vendor?.role === "customer" && (
                          <div className="flex justify-between text-[12px]">
                            ...
                            <span className="text-[#007185] font-medium">{customerOrders.length}</span>
                          </div>
                        )}
                      </div>
                ...
                {activeTab === "products" && (
                  <div className="space-y-2">
                    {productsLoading ? (
                      ...
                      <div key={p.id || `prod-${index}`} className="p-2.5 border border-zinc-200 rounded-md hover:border-[#007185] transition-all bg-white">
                        <div className="flex gap-2.5 mb-2">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-[12px] text-[#007185] font-semibold mt-0.5">د.أ {p.price}</p>
                          </div>
                        </div>
                        <button
                          onClick={() => handleSend(`استفسار عن: ${p.name}`, { type: "product", id: p.id, name: p.name, price: p.price, image: p.images?.[0]?.src })}
                          className="w-full h-[26px] bg-[#FFD814] hover:bg-[#F7CA00] border border-[#FCD200] rounded-md text-[11px] font-medium transition-all"
                        >
                          إرفاق بطاقة المنتج
                        </button>
                      </div>
                        ...
                        <div key={order.id || `order-${index}`} className="border border-zinc-200 rounded-md overflow-hidden">
                          ...
                          <div className="p-3 space-y-2">
                            ...
                            <button
                              ...
                              className="w-full h-[28px] bg-[#FFD814] hover:bg-[#F7CA00] border border-[#FCD200] rounded-md text-[11px] font-medium transition-all"
                            >
            ...
            <div className="p-5 border-b border-zinc-200">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full h-[34px] pl-9 pr-4 bg-zinc-50 border border-zinc-300 rounded-md text-[13px] outline-none focus:border-[#e77600] transition-all"
                />
              </div>
            </div>
                ...
                <div className="space-y-1">
                  {allVendors.filter((v, i, a) => a.findIndex(t => String(t.id) === String(v.id)) === i).map((v, index) => (
                    <div
                      ...
                      className={`flex items-center gap-3 p-3 rounded-md hover:bg-zinc-50 cursor-pointer group border border-transparent hover:border-zinc-200 transition-all ${v.role === "admin" ? "bg-[#e8f4f5]" : ""}`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-medium text-zinc-900 group-hover:text-[#007185] transition-colors">{v.storeName}</p>
                        <p className="text-[11px] text-zinc-400 mt-0.5">{v.role === "admin" ? "دعم النظام" : "تاجر رسمي"}</p>
                      </div>
                      <ChevronRight size={14} className="text-zinc-300 group-hover:text-[#007185] transition-all group-hover:translate-x-0.5" />
                    </div>
                  ))}
                </div>
              )}
