    ...
    return (
      <div className="min-h-screen bg-[#F8F9FA] flex flex-col items-center justify-center p-4">
        <div className="bg-white p-12 rounded-3xl border border-zinc-200 shadow-sm max-w-md w-full text-center">
          ...
          <Link href="/browse" className="inline-flex items-center gap-2 h-12 px-8 bg-[#FFD814] hover:bg-[#F7CA00] rounded-full text-sm font-bold text-zinc-900 transition-all">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative group">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-[#FFD814] transition-colors" size={14} />
              <input 
                type="text"
                placeholder="ابحث عن المنتجات..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="h-10 pl-10 pr-10 bg-white border border-zinc-200 rounded-xl text-sm focus:border-[#FFD814] focus:ring-1 focus:ring-[#FFD814] outline-none w-full md:w-64 transition-all"
              />
              {searchQuery && (
                <button 
        ...
        {filteredWishlist.length > 0 ? (
          ...
          <div className="py-32 bg-white rounded-[32px] border border-zinc-100 text-center shadow-sm">
            ...
            <button 
              ...
              className="mt-6 text-[#007185] font-bold text-sm hover:underline"
            >
              إعادة ضبط الفلاتر
            </button>
          </div>
