              ...
              <form onSubmit={handleRequest} className="p-8 flex-1 flex flex-col justify-between gap-8">
                 <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       <div className="space-y-2">
                          ...
                          <div className="relative group">
                             <span className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 font-bold text-[14px] group-focus-within:text-[#e77600] transition-colors">JOD</span>
                             <input 
                               type="number" 
                               step="0.01"
                               value={amount}
                               ...
                               placeholder="0.00"
                               className="w-full h-[52px] pl-14 pr-4 bg-zinc-50 border border-zinc-200 rounded-md text-[18px] font-bold outline-none focus:border-[#e77600] focus:ring-4 focus:ring-[#e77600]/5 focus:bg-white transition-all shadow-inner"
                               required
                             />
                          </div>
                       </div>
                          ...
                          <div className="flex gap-2">
                             {['bank', 'paypal'].map((m) => (
                               <button 
                                 ...
                                 type="button"
                                 onClick={() => setMethod(m)}
                                 className={`flex-1 flex items-center justify-center gap-2 h-[52px] border rounded-md transition-all ${method === m ? 'border-[#e77600] bg-orange-50 text-[#e77600] font-bold shadow-sm ring-1 ring-[#e77600]' : 'border-zinc-200 text-zinc-500 hover:bg-zinc-50'}`}
                               >
                    ...
                    {amount && parseFloat(amount) > 0 && (
                       <div className="p-5 bg-[#fff8ef] rounded-md border border-[#ffebcc] animate-in fade-in slide-in-from-top-1 duration-300">
                          <div className="flex justify-between items-center text-[13px] text-[#8a5b1a] mb-2">
                          ...
                          <div className="pt-2 border-t border-[#ffdfb3] flex justify-between items-center">
                             <span className="text-[14px] font-bold text-[#8a5b1a]">Estimated Receipt</span>
                             <span className="text-[18px] font-black text-[#8a5b1a]">
                                JOD {(() => {
                                   const charge = settings.charges?.[method];
                 ...
                 <button 
                   ...
                   className="w-full h-[52px] bg-[#FFD814] hover:bg-[#F7CA00] text-zinc-900 border border-[#FCD200] rounded-md text-[16px] font-black shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:grayscale transform active:scale-[0.98]"
                 >
