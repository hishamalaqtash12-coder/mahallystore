      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {plans.map((plan) => {
          ...
          return (
            ...
            >
              {isCurrent && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-[#007185] text-white text-[11px] font-bold uppercase tracking-wider rounded-full shadow-sm">
                  Current Plan
                </div>
              )}
              ...
              <div className="flex-1 space-y-3 mb-8">
                {plan.features.map((feature, idx) => (
                  <div key={idx} className="flex items-start gap-2">
                    <Check size={16} className={`mt-0.5 ${isCurrent ? 'text-[#007185]' : 'text-emerald-500'}`} />
              ...
              <button 
                ...
                className={`w-full py-2.5 rounded-lg text-[13px] font-bold transition-all ${
                  isCurrent 
                    ...
                    : 'bg-[#FFD814] hover:bg-[#F7CA00] text-zinc-900 border border-[#FCD200] shadow-sm'
                }`}
              >
