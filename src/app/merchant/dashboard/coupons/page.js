...
export default function CouponsPage() {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        setCoupons(data.map(c => ({
          ...
          usage_limit: parseInt(c.usage_limit || 0),
  ...
  const avgDiscount = coupons.length > 0
    ? coupons.reduce((sum, c) => sum + c.amount, 0) / coupons.length
    : 0;
  const usageRate = coupons.length > 0
    ? (coupons.filter(c => c.usage_count > 0).length / coupons.length) * 100
    : 0;
  ...
  return (
    <div className="space-y-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-[24px] font-bold text-zinc-900 tracking-tight">Coupons Management</h1>
          <p className="text-[13px] text-zinc-500 font-medium">Create and manage discount codes for your products</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: "Total Active", value: activeCount.toString(), color: "bg-emerald-50 text-emerald-700", icon: Ticket },
          { label: "Total Usage", value: totalUsage.toString(), color: "bg-blue-50 text-blue-700", icon: Activity },
          { label: "Avg. Discount", value: `${avgDiscount.toFixed(1)}${coupons[0]?.type === 'percent' ? '%' : ''}`, color: "bg-amber-50 text-amber-700", icon: Percent },
          { label: "Active Rate", value: `${usageRate.toFixed(0)}%`, color: "bg-purple-50 text-purple-700", icon: TrendingUp },
        ].map((stat, i) => (
            ...
            <tbody className="divide-y divide-zinc-100">
              {filteredCoupons.map((c) => (
                <tr key={c.id} className="hover:bg-zinc-50 transition-colors group">
                  ...
                  <td className="px-6 py-5 text-[13px] text-zinc-600 font-medium capitalize">
                    {c.type === 'percent' ? 'Percentage' : 'Fixed Amount'}
                  </td>
                  <td className="px-6 py-5 text-[14px] font-bold text-zinc-900">
                    {c.type === 'percent' ? `${c.amount}%` : `JOD ${c.amount.toFixed(2)}`}
                  </td>
                  <td className="px-6 py-5">
                  ...
                  <td className="px-6 py-5 text-center">
                    <div className="space-y-1">
                      <div className="w-24 h-1.5 bg-zinc-100 rounded-full mx-auto overflow-hidden">
                        <div
                          className={`h-full rounded-full ${c.status === 'expired' ? 'bg-zinc-300' : 'bg-[#e77600]'}`}
                  ...
                  <td className="px-6 py-5 text-center">
                    <span className={`px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${c.status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-rose-50 text-rose-700 border-rose-100'
                      }`}>
                      {c.status}
                    </span>
                  </td>
              ...
              {filteredCoupons.length === 0 && (
                <tr>
                  <td colSpan="7" className="px-6 py-20 text-center text-zinc-400 italic">No coupons found.</td>
                </tr>
              )}
            </tbody>
          </table>
