  ...
  if (loading) {
    return (
      <div className="flex-1 p-8 flex items-center justify-center bg-zinc-50 min-h-screen">
        <div className="w-8 h-8 border-4 border-zinc-200 border-t-[#007185] rounded-full animate-spin"></div>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm">
          <div>
            <h1 className="text-2xl font-bold text-zinc-900 flex items-center gap-2">
              <Megaphone size={24} className="text-[#e77600]" />
          ...
          {!showForm && (
            <button
              ...
              className="px-5 py-2.5 bg-[#FFD814] hover:bg-[#F7CA00] text-zinc-900 font-bold rounded-full shadow-sm text-[13px] transition-all flex items-center gap-2 border border-[#FCD200]"
            >
            ...
            <form onSubmit={handleCreateCampaign} className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div 
                      ...
                      className={`cursor-pointer p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2 text-center ${adType === "product" ? "border-[#007185] bg-[#e8f4f5]" : "border-zinc-200 hover:border-zinc-300 bg-white"}`}
                    >
                      <Package size={24} className={adType === "product" ? "text-[#007185]" : "text-zinc-400"} />
                      <span className={`text-[13px] font-bold ${adType === "product" ? "text-[#007185]" : "text-zinc-600"}`}>A Product</span>
                    </div>
                    <div 
                      onClick={() => setAdType("store")}
                      className={`cursor-pointer p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2 text-center ${adType === "store" ? "border-[#007185] bg-[#e8f4f5]" : "border-zinc-200 hover:border-zinc-300 bg-white"}`}
                    >
                      <Store size={24} className={adType === "store" ? "text-[#007185]" : "text-zinc-400"} />
                      <span className={`text-[13px] font-bold ${adType === "store" ? "text-[#007185]" : "text-zinc-600"}`}>My Entire Store</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[7, 14, 30].map(days => (
                      <div 
                        ...
                        className={`cursor-pointer p-3 rounded-xl border-2 transition-all flex flex-col items-center text-center ${duration === days ? "border-[#e77600] bg-[#fffaf5]" : "border-zinc-200 hover:border-zinc-300 bg-white"}`}
                      >
                        <span className={`text-[16px] font-black ${duration === days ? "text-[#e77600]" : "text-zinc-700"}`}>{days}</span>
                        <span className={`text-[11px] font-bold uppercase ${duration === days ? "text-[#e77600]" : "text-zinc-400"}`}>Days</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {adType === "product" && (
                <div className="space-y-2">
                  ...
                  <select 
                    ...
                    className="w-full h-11 px-4 rounded-lg border border-zinc-300 text-[13px] outline-none focus:border-[#007185] focus:ring-1 focus:ring-[#007185] bg-white"
                  >
              ...
              <div className="bg-zinc-50 p-5 rounded-xl border border-zinc-200 flex flex-col md:flex-row items-center justify-between gap-4 mt-6">
                <div>
                  ...
                  <p className="text-2xl font-black text-[#B12704]">JOD {AD_PRICING[duration].toFixed(2)}</p>
                </div>
                <div className="flex items-center gap-3 w-full md:w-auto">
                  ...
                  <button 
                    type="submit" 
                    disabled={submitting}
                    className="flex-1 md:flex-none px-6 py-2.5 bg-[#007185] hover:bg-[#005a6a] text-white font-bold rounded-lg shadow-sm text-[13px] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    ...
                    <tr key={camp.id} className="hover:bg-zinc-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="flex items-center gap-1.5 text-[12px] font-medium text-zinc-700 capitalize">
                          {camp.type === "store" ? <Store size={14} className="text-[#007185]"/> : <Package size={14} className="text-[#e77600]"/>}
                          {camp.type}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-[13px] font-bold text-[#B12704]">
                      ...
                      <td className="px-6 py-4">
                        {camp.status === "Pending Payment" ? (
                          <Link href={`/account/orders/${camp.id}`} className="text-[12px] font-bold text-[#007185] hover:text-[#e77600] flex items-center gap-1">
