    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-8">
         ...
         {recentViews.length > 0 && (
           <button onClick={clearAll} className="text-[13px] text-gray-500 font-bold hover:text-black hover:underline transition-all uppercase tracking-tighter">Clear all history</button>
         )}
      </div>
      ...
      {recentViews.length > 0 ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {recentViews.map((item) => (
            <div key={item.id} className="group relative flex flex-col bg-white border border-gray-100 rounded-md p-4 hover:shadow-lg transition-all hover:border-[#be374f]">
        ...
        <div className="py-24 flex flex-col items-center border border-gray-100 rounded-md bg-white text-center">
           ...
           <h3 className="text-[16px] font-bold mb-2 text-gray-900">History is empty</h3>
           <p className="text-gray-500 text-[14px] mb-8 max-w-xs">Items you view while browsing Mahally will appear here.</p>
           <Link href="/browse" className="px-10 py-3 bg-black text-white rounded-md font-bold text-[14px] transition-all hover:bg-gray-800">Start Browsing</Link>
        </div>
      )}
    </div>
  );
