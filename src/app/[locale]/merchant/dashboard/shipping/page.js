...
export default function MerchantShippingPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(isAr ? "فشل في حفظ إعدادات الشحن." : "Failed to save shipping settings.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900">{isAr ? "إعدادات الشحن" : "Shipping Settings"}</h1>
          <p className="text-zinc-500 text-sm">
            {isAr ? "حدد رسوم التوصيل لكل محافظة في الأردن." : "Configure delivery fees for each Jordan governorate."}
          </p>
        </div>
        <button
          onClick={handleSave}
        ...
        >
          ...
          {success ? (isAr ? "تم الحفظ!" : "Saved!") : (isAr ? "حفظ التغييرات" : "Save Changes")}
        </button>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-100 p-4 rounded-lg flex items-start gap-3 mb-8">
        ...
        <p className="text-[13px] text-blue-700 leading-relaxed">
          {isAr ? "حدد رسم توصيل ثابت لكل محافظة. إذا كنت تريد تقديم شحن مجاني فوق مبلغ معين (مثل شحن مجاني فوق 20 دينار أردني)، فادخل المبلغ في عمود \"شحن مجاني فوق\". اتركه فارغًا إذا لم تقدم شحنًا مجانيًا." : "Set a flat delivery fee for each governorate. If you want to offer free shipping above a certain order amount (e.g., Free delivery over 20 JOD), enter the amount in the \"Free Over\" column. Leave it blank if you don't offer free shipping."}
        </p>
      </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden shadow-sm">
        <table className="w-full text-end border-collapse">
          <thead className="bg-zinc-50 border-b border-zinc-200">
            <tr>
              <th className="px-6 py-4 text-[13px] font-bold text-zinc-600 uppercase">{isAr ? "المحافظة" : "Governorate"}</th>
              <th className="px-6 py-4 text-[13px] font-bold text-zinc-600 uppercase">{isAr ? "رسوم التوصيل (دينار)" : "Delivery Fee (JOD)"}</th>
              <th className="px-6 py-4 text-[13px] font-bold text-zinc-600 uppercase">{isAr ? "شحن مجاني فوق (دينار)" : "Free Over (JOD)"}</th>
            </tr>
          </thead>
      ...
      {success && (
        <div className="fixed bottom-8 start-8 bg-emerald-600 text-white px-6 py-3 rounded-lg shadow-2xl flex items-center gap-3 animate-in slide-in-from-bottom-5">
          <CheckCircle2 size={20} />
            <span className="font-medium">{isAr ? "تم تحديث إعدادات الشحن بنجاح!" : "Shipping settings updated successfully!"}</span>
        </div>
      )}
    </div>
  );
