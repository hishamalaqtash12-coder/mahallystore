...
export default function AccountFollowedStoresPage() {
  ...
  const fetchFollowedData = async () => {
    ...
    try {
      const [statusRes, vendorsRes] = await Promise.all([
        fetch(`/api/vendors/follow/status?userId=${wooId}&t=${Date.now()}`, { cache: 'no-store' }),
  ...
  const followedVendors = vendors.filter(v => followedIds.some(id => String(id) === String(v.id)));

  return (
    <div className="w-full">
      <h2 className="text-2xl font-bold mb-8 text-gray-900">Followed Stores</h2>
      ...
      {followedVendors.length === 0 ? (
        <div className="bg-white rounded-md border border-gray-100 p-16 flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-[16px] font-bold mb-2 text-gray-900">You haven't followed any stores yet</h3>
          <p className="text-gray-500 text-[14px] mb-8 text-center max-w-sm leading-relaxed">
            Follow your favorite stores to get updates on new arrivals and exclusive discounts.
          </p>
          <Link href="/vendors" className="px-10 py-3 bg-black text-white rounded-md font-bold text-[15px] hover:bg-gray-800 transition-all">
            Explore Stores
          </Link>
        </div>
      ) : (
            ...
            <div key={v.id} className="flex flex-col border border-zinc-200 rounded-md overflow-hidden hover:shadow-md transition-shadow bg-white relative">
              ...
              <div className="absolute top-14 end-4 w-12 h-12 rounded-md bg-white border border-zinc-200 shadow-md overflow-hidden p-0.5 flex items-center justify-center">
                {v.storeLogo ? (
                  ...
                  <span className="text-zinc-900 font-bold text-lg">{v.storeName[0]?.toUpperCase()}</span>
                )}
              </div>
              ...
              <div className="pt-6 pb-4 px-4 flex flex-col flex-1">
                <div className="flex justify-between items-start gap-4">
                  <div>
                    ...
                    {v.storeCategory && (
                      <span className="text-[11px] font-medium text-zinc-500 block mt-0.5">{v.storeCategory}</span>
                    )}
                  </div>
                  ...
                  >
                    {unfollowingId === v.id && <Loader2 className="animate-spin" size={10} />}
                ...
                <div className="flex items-center gap-1.5 mt-3">
                  ...
                  <span className="text-[11px] text-[#be374f]">
                    {v.rating > 0 ? `${v.rating.toFixed(1)}` : "New Store"}
                  </span>
                </div>
              </div>
            </div>
