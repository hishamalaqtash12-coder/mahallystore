...
const DEFAULT_SETTINGS = [
  { id: "promotions", meta: "Email & Push", enabled: true },
  { id: "orders", meta: "Email & SMS", enabled: true },
  { id: "chat", meta: "Email & In-app", enabled: true },
];
    ...
    if (notificationPreferences) {
      try {
        ...
        if (Array.isArray(parsed)) {
          // Merge with defaults to keep structure, only take enabled state from saved
      ...
      const res = await fetch("/api/auth/update-profile", {
        ...
        body: JSON.stringify({
          ...
          updates: {
            meta_data: [
              {
                key: "mahally_notification_preferences",
    ...
    <div className="w-full" dir={dir}>
      ...
      <div className="bg-white border border-gray-100 rounded-md overflow-hidden divide-y divide-gray-50 shadow-sm">
        {notificationSettings.map((item) => (
          <div
            key={item.id}
            className="p-6 flex items-start justify-between group hover:bg-gray-50/5 transition-all"
          >
            <div className="flex flex-col gap-0.5 max-w-2xl ps-8">
              <h3 className="text-[16px] font-bold text-gray-900 group-hover:text-[#be374f] transition-colors">
                {t(`${item.id}_title`)}
              </h3>
            ...
            >
              <div
                className={`absolute w-4 h-4 bg-white rounded-full shadow-sm transition-all ${item.enabled ? "end-7" : "end-1"
                  }`}
              />
            </button>
          </div>
      ...
      {isSaving && (
        <div className="fixed bottom-10 end-1/2 -translate-x-1/2 bg-black text-white px-8 py-3 rounded-md shadow-2xl flex items-center gap-3 animate-in slide-in-from-bottom-10 z-[100]">
          ...
          <span className="text-[14px] font-bold">{t("preferencesSaved")}</span>
        </div>
      )}
    </div>
  );
