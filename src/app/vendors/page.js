          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="relative w-full max-w-md">
              ...
              <input
                ...
                className="w-full h-11 bg-white border border-zinc-300 rounded-md pl-11 pr-4 text-[14px] outline-none focus:border-[#e77600] focus:ring-1 focus:ring-[#e77600] transition-all shadow-sm"
              />
            </div>
          </div>
        </div>
        ...
        <aside className="w-[240px] shrink-0 hidden md:block">
            ...
            {categories.map((c) => (
              <li key={c}>
                <button
                  ...
                  className={`text-[13px] w-full text-right py-1 hover:text-[#9b2c41] transition-colors ${catFilter === c ? "font-bold text-zinc-950" : "text-[#007185]"}`}
                >
                  {c === "All" ? "الكل" : c}
                </button>
              </li>
          ...
          <div className="space-y-1">
            {[4, 3, 2, 1].map(stars => (
              ...
              >
                ...
                <span className={`text-[13px] ${minRating === stars ? "font-bold text-[#9b2c41]" : "text-zinc-600 group-hover:text-[#9b2c41]"}`}>وأعلى</span>
              </button>
            ))}
          </div>
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 cursor-pointer group" onClick={() => setOnlyVerified(!onlyVerified)}>
              <div className={`w-4 h-4 border rounded flex items-center justify-center transition-colors ${onlyVerified ? 'bg-[#e77600] border-[#e77600]' : 'border-zinc-300'}`}>
                {onlyVerified && <CheckCircle size={10} className="text-white" />}
              </div>
              <span className="text-[13px] text-zinc-700 group-hover:text-[#9b2c41]">المتاجر الموثقة فقط</span>
            </label>
          </div>
          ...
          {!loading && filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center border border-dashed border-zinc-200 rounded-xl bg-zinc-50">
              ...
              <button
                ...
                className="mt-4 text-[13px] text-[#007185] font-medium hover:underline"
              >
                مسح جميع الفلاتر
              </button>
            </div>
                ...
                <div key={v.id} className="flex flex-col border border-zinc-200 rounded-md overflow-hidden hover:shadow-lg transition-shadow group bg-white">
                  ...
                  <div className="pt-8 pb-5 px-5 flex flex-col flex-1">
                    <Link href={`/vendors/${v.id}`} className="block">
                      <h2 className="text-[17px] font-bold text-zinc-900 hover:text-[#9b2c41] transition-colors leading-tight">
                        {v.storeName}
                      </h2>
                    </Link>
                    ...
                    <div className="flex items-center gap-2 mt-1 mb-3">
                      ...
                      <span className="text-[12px] text-[#007185] hover:text-[#9b2c41] cursor-pointer">
                    ...
                    <div className="mt-auto pt-4 border-t border-zinc-100 flex items-center justify-between">
                      <Link
                        href={`/vendors/${v.id}`}
                        className="text-[13px] font-medium text-[#007185] hover:text-[#9b2c41] hover:underline flex items-center gap-1"
                      >
                      ...
                      {Number(v.id) !== Number(wooId) && (
                        <button
                          ...
                          className={`cursor-pointer h-8 px-4 rounded-md text-[12px] font-medium shadow-sm transition-all border flex items-center justify-center gap-1.5 group ${followedStores.some(id => String(id) === String(v.id))
                              ...
                              : 'bg-[#FFD814] hover:bg-[#F7CA00] text-zinc-900 border-[#FCD200]'
